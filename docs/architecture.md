...
↓
Mock timers / localStorage для таймеров, закреплений и звука
↓
Supabase Auth + profiles для авторизации и роли
```

FastAPI backend skeleton уже создан, но пока не подключён к операциям с таймерами.
...
- `GET /health` — реализовано в backend skeleton.
...
- таймеры всё ещё работают через mock-данные в `timerService`;
- добавление, редактирование, удаление, restart и stop пока не идут через backend;
